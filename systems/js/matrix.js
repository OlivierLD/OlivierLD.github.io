...
function SquareMatrix(dim) {
	...
	this.setElementAt = (row, col, val) => {
		matrixElements[row][col] = val;
	};

	this.getElementAt = (row, col) => {
		return matrixElements[row][col];
	};

	this.getMatrixElements = () => {
		return this.matrixElements;
	};

	this.setMatrixElements = (me) => {
...
if (fromNode) {
	...
	/*
     ...
     Syntax for squareMatrix.setElementAt(row, col, value)	 
	 */
